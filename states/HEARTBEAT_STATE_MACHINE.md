...
Every heartbeat must do at least one of these in a meaningful way:
...
- `manager`
  - strategic steering across goals and planning structures such as milestones, epics, sprints, or simpler equivalents
...
- `cleanup`
  - standalone repo stewardship, artifact cleanup, and leftover classification
  - details: `states/cleanup.md`
- `refactoring`
...
- move to `manager` for milestone/epic/sprint steering, recurring planning, end-of-sprint review, or equivalent priority checkpoints
...
- move to `release` when the work is accepted and only release-specific cleanup, docs, or task updates remain
- move to `cleanup` for standalone repo hygiene, artifact cleanup, and leftover classification outside a specific release step
...
- if no concrete task is queued but active goals still exist, prefer `ponder`, `manager`, `cleanup`, or `refactoring` rather than idling
...
- using `cleanup` as disguised feature work or disguised architectural surgery
...
If `planning` already produced a concrete owner/file/function seam plus acceptance, validation, and next action, the next state must not be `planning` again unless new external information arrived, a binary user decision is needed, or the work is being explicitly frozen.

## State persistence
If the repo uses heartbeat memory, store runtime state in `memory/heartbeat_state.json`.
...
- `target`: worker-state field for the exact current work unit, may be null or omitted for `manager`, `ponder`, `sleep`, and other non-worker states
- `acceptance`: worker-state field for the measurable done condition, may be null or omitted for `manager`, `ponder`, `sleep`, and other non-worker states
- `task_ref`: optional pointer into a task, issue, or doc
...
- `last_sprint_plan_at`: timestamp of the most recent sprint or planning pass
- `sprint_due_at`: due date or time for the next planning or review checkpoint
...
- release or cleanup accepted work,
...
- stay quiet or handle the disablement through state or config only
...
- do not idle, prefer `ponder`, `manager`, `inspection_and_ideation`, `planning`, `cleanup`, `refactoring`, or an explicit freeze or handoff

## Repo note
For this repo, heartbeat is for bounded maintenance, planning, and small verified improvements.
