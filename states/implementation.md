...
- The work unit must stay singular:
  - one primary target
  - one primary validation command / sequence
...
- exact command(s) run
- pass / fail result
- if no file changed, provide concrete execution evidence (test/log output) or explicit freeze/user-decision path

## Enforcement (explicit)
- If no real blocker exists, implementation must include at least one execution step (tool call) that moves the work.
...
## Stop immediately if
- the first validation shows conceptual failure rather than syntax / integration failure
