...
1. Read `SOUL.md`
2. Read `USER.md`
3. Read recent memory in `memory/YYYY[-MM[-DD]].md` if this repo uses memory
...
- Keep `memory/YYYY-MM-DD.md` up to date with the current technical state after relevant work blocks when this repo uses memory.
...
If this repo uses memory, these files provide continuity:
...
- If no concrete implementation seam is ready, switch honestly to reflection, planning, review, cleanup, refactoring, or explicit freeze/handoff.
